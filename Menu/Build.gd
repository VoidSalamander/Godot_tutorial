...
func _input(event):
	if areaEnt == true:
		if event is InputEventScreenTouch and event.is_pressed():
			...
			if newDeltaX  < get_viewport_rect().size.x *0.8 and newDeltaY  > get_viewport_rect().size.y *0.62 and newDeltaX  > get_viewport_rect().size.x *0.52:
				if position.x > get_viewport_rect().size.x *0.8:
					set_position(Vector2(position.x, newDeltaY))
				elif position.y < get_viewport_rect().size.y *0.62:
					set_position(Vector2(newDeltaX, position.y))
			elif newDeltaX  <= get_viewport_rect().size.x *0.52:
				if newDeltaY <= get_viewport_rect().size.y *0.62:
					set_position(Vector2(position.x, newDeltaY))
			else:
